...
const TRACKING_ON = isEnabled("tracking");
...
test.describe("live tracking (Phase 7, Batch 7.4) @regression", () => {
  ...
  test("flag OFF: order-processing keeps the static confirmation (regression-safe)", async ({
    ...
    test.skip(TRACKING_ON, "flag is on — see the live-tracking test below");
  ...
  test("flag ON: subscribing over the cookie-auth socket shows a Live snapshot", async ({
    page,
    context,
    request,
  }) => {
    ...
    const cached = await page.evaluate((orderRequestId) => {
      const raw = window.localStorage.getItem("tracked-orders");
      const orders: { orderRequestId: string; fulfillmentId?: string }[] = raw
        ? JSON.parse(raw)
        : [];
      return orders.find((o) => o.orderRequestId === orderRequestId)?.fulfillmentId ?? null;
    }, orderRequestId);
    expect(cached).toBe(fulfillmentId);
  });
});
