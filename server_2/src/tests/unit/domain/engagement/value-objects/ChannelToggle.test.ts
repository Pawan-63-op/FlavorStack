...
describe('ChannelToggle', () => {
  it('creates with explicit push/email flags', () => {
    const result = ChannelToggle.create({ push: true, email: false });
    expect(result.isSuccess).toBe(true);
    const toggle = result.getValue();
    expect(toggle.isEnabled(NOTIFICATION_CHANNEL.PUSH)).toBe(true);
    expect(toggle.isEnabled(NOTIFICATION_CHANNEL.EMAIL)).toBe(false);
  });

  it('rejects non-boolean flags', () => {
    const result = ChannelToggle.create({ push: 'yes' as unknown as boolean, email: false });
    expect(result.isFailure).toBe(true);
  });

  it('withChannel returns a new toggle with the channel flipped', () => {
    const toggle = ChannelToggle.create({ push: true, email: true }).getValue();
  ...
  it('allBoth() / pushOnly() factories', () => {
    expect(ChannelToggle.allEnabled().isEnabled(NOTIFICATION_CHANNEL.PUSH)).toBe(true);
    expect(ChannelToggle.allEnabled().isEnabled(NOTIFICATION_CHANNEL.EMAIL)).toBe(true);
    expect(ChannelToggle.pushOnly().isEnabled(NOTIFICATION_CHANNEL.PUSH)).toBe(true);
    expect(ChannelToggle.pushOnly().isEnabled(NOTIFICATION_CHANNEL.EMAIL)).toBe(false);
  });
});
