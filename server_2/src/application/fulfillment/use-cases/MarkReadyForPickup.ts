...
export class MarkReadyForPickup {
  ...
  async execute(dto: MarkReadyForPickupDto): Promise<Result<FulfillmentResponse>> {
    ...
    if (!ownerId || ownerId !== dto.actorUserId) {
      return Result.fail(new ForbiddenError('Only the owning restaurant can mark ready for pickup'));
    }

    const result = fulfillment.markReadyForPickup(fulfillment.restaurantId);
