...
import { ReorderCategoriesDto } from '../dtos/ReorderCategoriesDto';
...
/** Owner (or super-admin) write: re-sorts the restaurant's categories. */
export class ReorderCategories {
  ...
  async execute(dto: ReorderCategoriesDto): Promise<Result<RestaurantResponse>> {
    ...
    const reorderResult = restaurant.reorderCategories(dto.orderedCategoryIds);
    if (reorderResult.isFailure) return Result.fail(reorderResult.getError());
