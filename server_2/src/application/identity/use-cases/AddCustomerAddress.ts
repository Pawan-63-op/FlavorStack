...
import { AddAddressDto } from '../dtos/AddressDtos';
...
export class AddCustomerAddress {
  ...
  async execute(dto: AddAddressDto): Promise<Result<AddressResponse[]>> {
    ...
    const id = user.addAddress(addr.getValue());
    if (dto.isDefault) user.setDefaultAddress(id);
