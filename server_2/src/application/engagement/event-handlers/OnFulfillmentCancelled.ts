// Event handler: consumes Fulfillment's FulfillmentCancelled → an ORDER_UPDATES push notification
// (engagement_module.md §3/§4). The event carries no customerId, so the recipient is resolved from the
// ReviewEligibility projection seeded at FulfillmentCreated — no Fulfillment/Commerce coupling. The
// cancellation `reason` is forwarded as a template var.
//
