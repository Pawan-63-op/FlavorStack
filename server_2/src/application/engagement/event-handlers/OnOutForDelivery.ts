// Event handler: consumes Fulfillment's OutForDelivery → a DELIVERY push notification
  ...
  async handle(event: DomainEvent): Promise<void> {
    ...
    if (!eligibility) {
      logger.warn(
        ...
        '[OnOutForDelivery] no eligibility seeded yet — skipping (allow retry)'
      );
      return;
    }
    ...
    if (result.isFailure) {
      logger.error(
        ...
        '[OnOutForDelivery] dispatch failed'
      );
      return;
    }
